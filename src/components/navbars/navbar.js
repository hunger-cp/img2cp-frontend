...
export default function Navbar(props) {
  ...
  useEffect(() => {
    setPfpURL(getAuth().currentUser.photoURL)
  }, [])
  ...
  //             <p className="text-white mr-3"> Welcome {getAuth().currentUser.displayName.split(" ")[0]}! </p>
  ...
  //               </span>
  //               <button className="ml-3 bg-zinc-800 active:bg-zinc-700 text-gray-100 font-normal px-4 py-2 rounded outline-none focus:outline-none mr-1 mb-1 uppercase shadow hover:shadow-md inline-flex items-center font-bold text-xs ease-linear transition-all duration-150"
  //             onClick = {signOut}>
  //               Sign Out
  //             </button>
  //             </div>
      ...
      <nav className="absolute top-0 left-0 w-full z-10 bg-transparent md:flex-row md:flex-no-wrap md:justify-start flex items-center p-4">
        <div className="w-full mx-autp items-center flex justify-between md:flex-no-wrap flex-wrap md:px-10 px-4">
          ...
          <ul className="flex-col md:flex-row list-none items-center hidden md:flex">
            <a className="text-gray-600 block">
              <div className="items-center flex">
              <p className="text-zinc-700 font-semibold mr-3"> {getAuth().currentUser.displayName == null ? getAuth().currentUser.email : getAuth().currentUser.displayName} </p>
              {/* <p className="text-black mr-3"> {firebase.auth().currentUser.displayName}</p> */}
                <span className="w-12 h-12 text-sm text-white bg-gray-300 inline-flex items-center justify-center rounded-full">
                {pfpURL == "" ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" ><path d="M16 .7C7.56.7.7 7.56.7 16S7.56 31.3 16 31.3 31.3 24.44 31.3 16 24.44.7 16 .7zm0 28c-4.02 0-7.6-1.88-9.93-4.81a12.43 12.43 0 0 1 6.45-4.4A6.5 6.5 0 0 1 9.5 14a6.5 6.5 0 0 1 13 0 6.51 6.51 0 0 1-3.02 5.5 12.42 12.42 0 0 1 6.45 4.4A12.67 12.67 0 0 1 16 28.7z"></path></svg>
                :<img className="rounded-full" src={pfpURL}></img>  }         </span>
